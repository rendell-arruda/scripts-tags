...
if [ -f "$arquivo_arns_cluster_rds" ]; then
    ...
    while IFS= read -r arn_cluster_rds || [[ -n $arn_cluster_rds ]]; do
        output=$(aws rds add-tags-to-resource --resource-name "${prefix_arn}${arn_cluster_rds}" --tags Key=Produto,Value=infraestrutura Key=Subproduto,Value=infra Key=finops.subproduto,Value=infra2 2>&1)
