...
if [ -f "$arquivo_grupos_logs" ]; then
    ...
    while IFS= read -r nome_grupo_logs || [[ -n $nome_grupo_logs ]]; do
        output=$(aws logs tag-log-group --log-group-name "$nome_grupo_logs" --tags Key=Produto,Value=infraestrutura Key=Subproduto,Value=infra Key=finops.subproduto,Value=infra2 2>&1)
