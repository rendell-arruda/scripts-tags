...
prefix_arn=("arn:aws:rds:us-east-1:266549158321:db:")

# Loop para adicionar as tags às instâncias de banco de dados do RDS
for instancia_rds in "${nomes_rds[@]}"; do
    output=$(aws rds add-tags-to-resource --resource-name "${prefix_arn}${instancia_rds}" --tags Key=Produto,Value=infraestrutura Key=Subproduto,Value=infraestrutura 2>&1)
