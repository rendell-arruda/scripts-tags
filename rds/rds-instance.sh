...
if [ -f "$arquivo_arns_instance_rds" ]; then
    ...
    while IFS= read -r arn_instance_rds || [[ -n $arn_instance_rds ]]; do
        output=$(aws rds add-tags-to-resource --resource-name "${prefix_arn}${arn_instance_rds}" --tags Key=Produto,Value=infraestrutura Key=Subproduto,Value=infraestrutura 2>&1)
