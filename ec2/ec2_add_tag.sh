...
if [ -f "$arquivo_ids" ]; then
    ...
    while IFS= read -r id_ec2 || [[ -n $id_ec2 ]]; do
        output=$(aws ec2 create-tags --resources "$id_ec2" --tags Key=Produto,Value=infraestrutura Key=Subproduto,Value=infra Key=finops.subproduto,Value=infra2 Key=finops.isso,Value=infra4 2>&1)
